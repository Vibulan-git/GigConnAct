...
class StateManager {
    ...
    async confirmPendingRegistration() {
        ...
        const newUser = {
            ...
            subscriptionPlan: pendingReg.subscriptionPlan || "free",
            successfulGigs: 0,
            contactRequests: 0,
            favorites: [],
            interests: []
...
function showToast(options) {
    ...
    const isOrg = (state && state.currentUser && state.currentUser.role === 'organizer');
    const bellColor = isOrg ? '#2563eb' : 'var(--color-purple)';

    const toast = document.createElement('div');
    toast.className = 'toast';
    toast.innerHTML = `
        <div class="toast-icon" style="color: ${bellColor} !important;"><i class="fa-solid fa-bell"></i></div>
        ...
        <div class="market-page ${isEvents ? 'theme-musician' : 'theme-organizer'}" style="max-width: 1520px; margin: 0 auto; padding: 1.5rem 1rem 5rem; box-sizing: border-box;">
            ...
            <div class="market-layout-container">
                ...
                <div id="market-filters-wrapper" class="market-filter-card">
                    ...
                    ${isEvents ? `
                        <!-- 11 Event-Markt Filter + Suchbegriffe mit tag-pill-checkboxes & dual range sliders -->
                    ...
                    ` : `
                        <!-- 11 Musiker-Markt Filter + Suchbegriffe mit tag-pill-checkboxes & dual range sliders -->
                        <div style="display: flex; flex-direction: column; gap: 0.8rem; padding: 1rem;">
                            ...
                            <!-- 5. Gesuchte Event-Typen -->
                            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 0.8rem;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.35rem;">
                                    <label style="font-size: 0.85rem; font-weight: 900; color: #2563eb; margin: 0;">Gesuchte Event-Typen</label>
                                    <div style="display: flex; gap: 0.5rem; font-size: 0.75rem; font-weight: 700;">
                                        <span onclick="window.toggleAllFilterCheckboxes(this, true)" style="color: #2563eb; cursor: pointer; text-decoration: underline;">alle auswählen</span>
                                    </div>
                                </div>
...
window.revealMarketContact = async function(itemId, type, value, clickedBtn) {
...
function showSubscriptionChangeSuccessModal() {
    ...
    modal.querySelector('#btn-close-sub-success-modal').addEventListener('click', () => {
        modal.remove();
    });
}
window.showSubscriptionChangeSuccessModal = showSubscriptionChangeSuccessModal;
        ...
        <div class="market-tile-card" style="background: var(--bg-card); border: 1px solid var(--border-glass); border-radius: 18px; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; box-shadow: var(--shadow-sm); opacity: ${isActive ? '1' : '0.75'};">
            ...
            <div class="tile-body-content" style="padding: 1.2rem 1.1rem 1.0rem; flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
                <div>
                    ...
                    <!-- 2-Column Layout matching market: Left = first 6 fields, Right = Plus expand button -->
                    <div style="display: flex; gap: 0.5rem; justify-content: space-between;">
                        ...
                        <!-- Right Column: Plus (Expand Button) -->
                        <div style="display: flex; flex-direction: column; justify-content: flex-end; padding-left: 0.6rem; border-left: 1px solid var(--border-glass); padding-bottom: 0.1rem; min-width: 46px; box-sizing: border-box; align-items: center;">
                            <button onclick="event.stopPropagation(); window.toggleTileDetails('${e.id}')" style="background: none; border: none; padding: 0; cursor: pointer; color: ${themeColor}; outline: none; transition: transform 0.2s; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'" title="Mehr Details anzeigen/verbergen">
                                <i class="fa-solid fa-circle-plus" id="toggle-icon-${e.id}" style="font-size: 1.6rem; opacity: 0.85;"></i>
                            </button>
                        </div>
                    </div>

                    <!-- Collapsible details wrapper -->
                    <div id="collapsible-details-${e.id}" style="display: none; flex-direction: column; gap: 0.5rem; border-top: 1px dashed var(--border-glass); padding-top: 0.5rem; margin-top: 0.2rem; margin-bottom: 0.75rem;">
                        <!-- 7. Spielzeit -->
                ...
                <form id="musician-editor-form">
                    ...
                    <div class="form-group">
                        <div class="slider-value-display">
                            ...
                            <span id="val-radius">${musicianObj?.radius !== undefined ? musicianObj.radius : 150}</span>
                        </div>
                        <input type="range" name="radius" min="0" max="500" step="50" value="${musicianObj?.radius !== undefined ? musicianObj.radius : 150}" class="input-field" style="padding:0; height:auto; accent-color:#a855f7;">
                    </div>
                    ...
                    <div class="form-group">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.35rem;">
                            <label style="margin: 0;">Gesuchte Event-Typen</label>
        ...
        <div class="modal-content" style="max-width: 650px; max-height: 85vh; overflow-y: auto; text-align: left;">
            ...
            <div class="modal-body">
                <form id="event-editor-form">
                    ...
                    <div style="display: flex; align-items: flex-start; gap: 0.5rem; margin-top: 1.2rem; margin-bottom: 0.8rem;">
                        <input type="checkbox" id="chk-event-publish-consent" style="width: auto; margin-top: 0.25rem; cursor: pointer;" required checked>
                        <label for="chk-event-publish-consent" style="font-size: 0.75rem; font-weight: normal; color: var(--text-muted); cursor: pointer; margin: 0; line-height: 1.35; text-align: left;">
                            GigConnAct hat deine Erlaubnis, dein Event auf GigConnAct zu veröffentlichen.
                        </label>
                    </div>
...
function renderAuthModal(wrapper, onSuccessCallback, defaultRole) {
                ...
                <form id="auth-register-form" class="hidden">
                    ...
                    <div id="reg-fields-musician">
                        ...
                        <div class="form-group">
                            ...
                            <input type="range" name="radius" min="0" max="500" step="50" value="150" class="input-field" style="padding:0; height:auto; accent-color:#a855f7;">
                        </div>
                    ...
                    <div id="reg-event-publish-consent-container" style="display: none; margin-top: 0.5rem; margin-bottom: 1.2rem;">
                        <label class="form-checkbox" style="display: flex; align-items: flex-start; gap: 0.6rem; font-size: 0.8rem; line-height: 1.4; color: var(--text-muted); cursor: pointer;">
                            ...
                            <span>GigConnAct hat deine Erlaubnis, dein Event auf GigConnAct zu veröffentlichen.</span>
                        </label>
                    </div>
            ...
            if (!res.success) {
                ...
                if (res.message === "Diese E-Mail-Adresse ist nicht registriert.") {
                    ...
                    if (registerTabBtn) {
                        ...
                        const regEmailInput = document.querySelector('#auth-register-form input[name="email"]');
                        if (regEmailInput) {
                            regEmailInput.value = email;
                        }
                        showToast({
                            title: "Konto nicht gefunden 🔍",
                            message: "Diese E-Mail-Adresse ist noch nicht registriert. Bitte erstelle zuerst ein Konto."
                        });
                    }
                } else {
                    if (errDiv) {
    ...
    // Default class on form and initialize publish permission consent visibility
    if (registerForm) {
        const publishConsentContainer = document.getElementById('reg-event-publish-consent-container');
        if (selectedRole === 'organizer') {
            registerForm.classList.add('role-organizer-active');
            registerForm.classList.remove('role-musician-active');
            if (publishConsentContainer) {
                publishConsentContainer.style.display = 'block';
                const chk = publishConsentContainer.querySelector('input');
                if (chk) {
                    chk.setAttribute('required', '');
                    chk.checked = true;
                }
            }
        } else {
            ...
            if (publishConsentContainer) {
                publishConsentContainer.style.display = 'none';
                const chk = publishConsentContainer.querySelector('input');
                if (chk) {
    ...
    if (el && icon) {
        ...
        if (isCollapsed) {
            ...
            if (container) {
                ...
                const dotsSpans = container.querySelectorAll('[id^="dots-"]');
...
function formatTruncatedValue(val, themeColor, itemId, uniqueType) {
    ...
    const dotsId = `dots-${uniqueType}-${itemId}`;
    
    return `
        <span>${visiblePart}</span><span id="${dotsId}">...</span><span id="${hiddenId}" style="display: none;">${hiddenPart}</span>
    `;
}
...
window.showMatchmakingChoiceModal = function(musicianId, bandName) {
    ...
    overlay.style = "position:fixed; inset:0; background:rgba(0,0,0,0.8); z-index:99999; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(6px); padding:1rem;";
    
    overlay.innerHTML = `
        <div style="width:100%; max-width:680px; background:var(--bg-card); border:1px solid var(--border-glass); border-radius:18px; padding:2rem; box-shadow:0 15px 35px rgba(0,0,0,0.4); position:relative; font-family:var(--font-heading); color:#fff;">
            
            <button id="btn-close-choice-modal" style="position:absolute; top:1.2rem; right:1.2rem; background:transparent; border:none; color:var(--text-muted); cursor:pointer; font-size:1.4rem;">
                <i class="fa-solid fa-xmark"></i>
            </button>

            <h3 style="font-size:1.4rem; font-weight:800; text-align:center; margin:0 0 1.5rem; color:#fff;">Kontaktdaten freischalten</h3>
            <p style="font-size:0.9rem; color:var(--text-muted); text-align:center; margin-bottom:2rem; line-height:1.4;">
                Wie möchtest du mit dem Künstler in Verbindung treten? Wähle eine der folgenden Optionen:
            </p>

            <div class="choice-cards-container" style="display:grid; grid-template-columns:1fr; gap:1.5rem;">
                <style>
                ...
                <div style="background:rgba(255,255,255,0.02); border:1px solid var(--border-glass); border-radius:12px; padding:1.5rem; display:flex; flex-direction:column; justify-content:space-between;">
                    <div>
                        <div style="font-size:2rem; color:#3b82f6; margin-bottom:0.75rem; text-align:center;">
                            <i class="fa-solid fa-user-plus"></i>
                        </div>
                        <h4 style="font-size:1.05rem; font-weight:700; margin:0 0 0.75rem; text-align:center;">1. Selbst kontaktieren (Kostenlos)</h4>
                        <ul style="font-size:0.8rem; color:var(--text-muted); padding-left:1.1rem; margin:0 0 1.5rem; line-height:1.6;">
                            <li>Kostenloses Veranstalter-Konto erstellen</li>
                            <li>Beliebig viele Musiker selbst anfragen und Verträge aushandeln</li>
                            <li>Direkte Kommunikation im Chat</li>
                        </ul>
                    </div>
                    <button id="btn-choice-register" class="btn btn-primary" style="width:100%; margin:0; padding:0.75rem; font-size:0.85rem; font-weight:700; background:#3b82f6; border-color:#3b82f6;">
                        Kostenlos registrieren
                    </button>
                </div>

                <!-- Weg 2: GigConnAct-Vermittlung -->
                <div style="background:rgba(124,58,237,0.05); border:1px solid rgba(124,58,237,0.25); border-radius:12px; padding:1.5rem; display:flex; flex-direction:column; justify-content:space-between; position:relative; box-shadow:0 0 15px rgba(124,58,237,0.1);">
                    <div style="position:absolute; top:-10px; right:15px; background:#7c3aed; color:#fff; font-size:0.6rem; font-weight:900; padding:0.2rem 0.5rem; border-radius:10px; text-transform:uppercase; letter-spacing:0.5px;">Bequem & Sicher</div>
                    <div>
                        ...
                        <ul style="font-size:0.8rem; color:var(--text-muted); padding-left:1.1rem; margin:0 0 1.5rem; line-height:1.6;">
                            ...
                            <li>Provision erst fällig bei erfolgreichem Auftritt</li>
                        </ul>
                    </div>
...
window.showAgencyBookingForm = function(musicianId, bandName) {
    ...
    const instrumentsGrid = document.getElementById('filter-instruments-grid-m');
    if (instrumentsGrid) {
        filterInstruments = Array.from(instrumentsGrid.querySelectorAll('input:checked')).map(el => el.value);
    }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding: 1.5rem 2rem; max-height: 70vh; overflow-y: auto;">
                ...
                <form id="auth-register-form" class="role-organizer-active">
                    ...
                    <div style="display: flex; align-items: flex-start; gap: 0.5rem; margin-top: 1.2rem; margin-bottom: 0.8rem;">
                        <input type="checkbox" id="chk-agency-publish-consent" style="width: auto; margin-top: 0.25rem; cursor: pointer;" required checked>
                        <label for="chk-agency-publish-consent" style="font-size: 0.75rem; font-weight: normal; color: var(--text-muted); cursor: pointer; margin: 0; line-height: 1.35; text-align: left;">
                            GigConnAct hat deine Erlaubnis, dein Event auf GigConnAct zu veröffentlichen.
                        </label>
                    </div>
...
function showAgencySuccessModal(email) {
    ...
    overlay.innerHTML = `
        <div style="width:100%; max-width:440px; background:var(--bg-card); border:1px solid var(--border-glass); border-radius:16px; padding:2.5rem 2rem; text-align:center; box-shadow:0 15px 35px rgba(0,0,0,0.4); position:relative; font-family:var(--font-heading); color:#fff;">
            <div style="font-size:4rem; color:#10b981; margin-bottom:1.2rem; filter:drop-shadow(0 0 8px rgba(16,185,129,0.3));">
                <i class="fa-solid fa-circle-check"></i>
            </div>
            <h3 style="font-size:1.4rem; color:#fff; margin-bottom:0.75rem;">Anfrage gesendet! 🚀</h3>
            <p style="font-size:0.9rem; color:var(--text-muted); line-height:1.55; margin-bottom:1.8rem;">
                Vielen Dank! Deine Vermittlungsanfrage ist bei uns eingegangen. Wir prüfen die passenden Acts und senden dir innerhalb der nächsten 5 Werktage per E-Mail an <strong>${email}</strong> unsere handverlesene Auswahl zu.
            </p>
            <button id="btn-close-agency-success" class="btn btn-primary" style="width:100%; padding:0.85rem; font-size:1rem; font-weight:700; border-radius:10px; background:#10b981; border-color:#10b981; margin:0;">
                Schließen
            </button>
        </div>
    `;
            ...
            <div class="market-tile-card" style="cursor: default; background: var(--bg-card); border: 1px solid var(--border-glass); border-radius: 18px; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; box-shadow: var(--shadow-sm);">
                ...
                <div class="tile-fullwidth-photo-slider" style="position: relative; width: 100%; height: 235px; background: #0f172a; overflow: hidden;">
                    ...
                    ${(state.currentUser && item.matchScore >= 70) ? `
                        <div class="tile-top-match-badge" title="Top Match" style="position: absolute; top: 12px; left: 12px; z-index: 5; background: rgba(15, 23, 42, 0.75); border: 1px solid rgba(254, 240, 138, 0.35); border-radius: 12px; padding: 0.25rem 0.55rem; display: flex; align-items: center; gap: 0.3rem; backdrop-filter: blur(4px); pointer-events: none;">
                            <i class="fa-solid fa-star" style="color: #eab308; font-size: 0.85rem; margin: 0;"></i>
                            <span style="font-size: 0.7rem; font-weight: 800; color: #fef08a; font-family: var(--font-heading); line-height: 1;">${item.matchScore}%</span>
                        </div>
                    ` : ''}
                ...
                <div class="tile-body-content" style="padding: 1.3rem 1.3rem 0.8rem; flex: 1; display: flex; flex-direction: column;">
                    ...
                    <div style="margin-bottom: 0.8rem;">
                        <h3 style="font-family: var(--font-heading); font-size: 1.25rem; font-weight: 800; color: var(--text-main); margin: 0; line-height: 1.2; height: 2.4em; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; word-break: break-word;">
                            ${displayName}
                        </h3>
                    </div>
            ...
            <div style="padding: 1.5rem; flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 1.2rem;">
                ...
                <div>
                    <h4 style="margin: 0 0 0.6rem; font-size: 0.9rem; color: var(--text-main); display: flex; justify-content: space-between; align-items: center;">
                        <span style="display: inline-flex; align-items: center; gap: 0.3rem;">📷 Fotos (${photos.length}/5) <i class="fa-solid fa-circle-info" style="cursor: pointer; color: var(--text-muted); font-size: 0.8rem;" title="Erlaubte Formate: JPG, JPEG, PNG, WebP&#10;Maximale Größe: 10 MB&#10;Auflösung: mind. 1200 x 1200 px"></i></span>
                        ${photos.length < 5 ? `
        ...
        <div class="market-container" style="max-width: 800px; margin: 3rem auto; padding: 2.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-color); box-shadow: var(--shadow-lg);">
            ...
            <div style="line-height: 1.8; color: var(--text-main); font-size: 0.95rem;">
                ...
                <p>
                    ...
                    Gekündigte Profile verbleiben nach Ablauf des Abonnement-Zeitraums in einem inaktiven Zustand (inaktives Benutzerkonto), um dem Nutzer eine spätere Reaktivierung zu ermöglichen und den Missbrauch von kostenlosen Probephasen zu verhindern. Sie können Ihr Konto und alle hiermit verbundenen Daten jederzeit selbstständig über die Funktion „Konto löschen“ im Profilbereich unwiderruflich entfernen. Zur Verhinderung von Missbrauch kostenloser Testphasen speichern wir im Falle einer Kontolöschung den unumkehrbaren kryptografischen Hashwert (SHA-256) Ihrer E-Mail-Adresse dauerhaft auf Grundlage unseres berechtigten Interesses gemäß Art. 6 Abs. 1 lit. f DSGVO. Eine Zuordnung zu Ihrer Person oder eine Rekonstruktion der E-Mail-Adresse ist aus diesem Hashwert nicht möglich.
                </p>
